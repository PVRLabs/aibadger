...
Need more context for unusually large repositories or source files? Badger's
interactive limits can be increased in `~/.badger/settings.json`. See
[Settings and External Context](settings.md) for the available limits and
ranges.

## External Context

For read-only external directories, see [Settings and External
