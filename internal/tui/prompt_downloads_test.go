...
func TestNormalPromptViewsOfferDownloads(t *testing.T) {
	...
	if view := m.viewScanComplete(); !strings.Contains(view, "[Enter/Y] Clipboard  [D] Downloads  [N] Skip") {
		t.Fatalf("Prompt 1 view missing Downloads choices:\n%s", view)
	}
	if strings.Contains(m.viewScanComplete(), "(y/N)") || !strings.Contains(m.viewScanComplete(), "[Enter/Y] Clipboard") {
	...
	if view := m.viewContextReady(); !strings.Contains(view, "[Enter/Y] Clipboard  [D] Downloads  [N] Skip") {
		t.Fatalf("Prompt 2 view missing Downloads choices:\n%s", view)
	}
	if strings.Contains(m.viewContextReady(), "(y/N)") || !strings.Contains(m.viewContextReady(), "[Enter/Y] Clipboard") {
