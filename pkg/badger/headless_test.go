...
func TestHandleFinalResponseUsesGenericHeadlessCopyInstruction(t *testing.T) {
	...
	var output bytes.Buffer
	if handleFinalResponse(&output, bufio.NewReader(strings.NewReader("\n")), session, HeadlessOptions{
		Stdin: strings.NewReader("No file updates needed.\nDONE\n"),
	}) {
		t.Fatal("handleFinalResponse() returned true, want false")
	}

	for _, want := range []string{
		"1. Paste Prompt 2: Code Context into your AI chat.",
		"2. Copy the final response generated by AI.",
		"3. Paste it below and type 'DONE' on a new line to finish.",
	} {
		if !strings.Contains(output.String(), want) {
			t.Fatalf("handleFinalResponse() output missing %q:\n%s", want, output.String())
		}
	}
	if strings.Contains(output.String(), "final code generated by AI") {
		t.Fatalf("handleFinalResponse() output still used code-only wording:\n%s", output.String())
	}
}
