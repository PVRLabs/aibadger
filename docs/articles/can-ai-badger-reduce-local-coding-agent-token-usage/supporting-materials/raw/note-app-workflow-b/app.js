...
const els = {
  ...
  settingsSortSelect: document.getElementById('settingsSortSelect'),
...
function loadUI() {
  ...
  return {
    ...
    theme: ['system', 'light', 'dark'].includes(ui.theme) ? ui.theme : 'system',
...
function applyTheme() {
  const resolved = state.ui.theme === 'system' ? (systemTheme.matches ? 'dark' : 'light') : state.ui.theme;
  document.documentElement.dataset.theme = resolved;
}

function openSettingsModal() {
  els.settingsSortSelect.value = state.ui.sort;
  els.themeInputs.forEach(input => {
    input.checked = input.value === state.ui.theme;
  });
  els.settingsModal.classList.remove('hidden');
  const checked = [...els.themeInputs].find(input => input.checked);
...
els.settingsSortSelect.addEventListener('change', () => {
  state.ui.sort = els.settingsSortSelect.value;
  saveUI();
  render();
});
...
systemTheme.addEventListener('change', () => {
  if (state.ui.theme === 'system') applyTheme();
});
