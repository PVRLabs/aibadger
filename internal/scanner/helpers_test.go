...
func TestReviewPathPriorityPathCategories(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "C++ cc source path", relPath: filepath.Join("src", "parser.cc"), priority: 1, ok: true},
		{name: "C++ cxx source path", relPath: filepath.Join("src", "parser.cxx"), priority: 1, ok: true},
		{name: "C++ hh header path", relPath: filepath.Join("include", "parser.hh"), priority: 1, ok: true},
		{name: "C++ hxx header path", relPath: filepath.Join("include", "parser.hxx"), priority: 1, ok: true},
